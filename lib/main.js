...
async function generateSharingToken (reporter, shortid) {
  const templates = await reporter.documentStore.collection('templates').find({shortid: shortid})
  if (templates.length < 1) {
    throw new Error('Unable to find tempalte with shortid: ' + shortid)
  }
...
function routes (reporter) {
  return (app) => {
    app.get('/api/templates/sharing/:shortid/grant/:access', (req, res, next) => {
      generateSharingToken(reporter, req.params.shortid, req.params.access).then((token) => {
        res.send({
          token: token
        })
      }).catch(next)
    ...
    app.get('/public-templates', (req, res, next) => {
      reporter.documentStore.collection('templates').find({readSharingToken: req.query.access_token}).then((templates) => {
        if (templates.length !== 1) {
