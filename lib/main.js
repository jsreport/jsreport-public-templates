...
async function generateSharingToken (reporter, shortid, req) {
  const templates = await reporter.documentStore.collection('templates').find({shortid: shortid}, req)
...
function routes (reporter) {
  return (app) => {
    app.post('/api/templates/sharing/:shortid/access/:access', (req, res, next) => {
      generateSharingToken(reporter, req.params.shortid, req.params.access, req).then((token) => {
        res.send({
          token: token
        })
      }).catch(next)
