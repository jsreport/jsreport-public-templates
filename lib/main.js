...
PublicTemplates.prototype.configureExpress = function (app, reporter) {
  ...
  app.get('/public-templates', function (req, res, next) {
    ...
    req.context.skipAuthorizationForQuery = query
  ...
  reporter.initializeListeners.add('public-templates', function () {
    ...
    function verifyToken (req, query, token) {
      ...
      return reporter.documentStore.collection('templates').find(query).then(function (templates) {
        ...
        req.context.tokenVerified = true
        req.context.skipAuthorizationForUpdate = query
      })
    }
    ...
    function addHooksToCollection (col) {
      col.beforeUpdateListeners.insert(0, 'public-templates', col, function (query, update, req) {
        ...
        if (this.name === 'templates' && !req.context.user && update.$set && update.$set.writeSharingToken) {
      ...
      col.beforeInsertListeners.insert(0, 'public-templates', col, function (doc, req) {
        ...
        if (this.name === 'templatesHistory') {
          req.context.skipAuthorizationForInsert = doc
        }
      })

      col.beforeFindListeners.add('public-templates', col, function (query, projection, req) {
        if (!req) {
          return
        }
        ...
        if (req.context.user || _.isEqual(req.context.skipAuthorizationForQuery, query) || req.context.tokenVerified) {
          return
        }
